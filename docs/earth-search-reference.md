# Earth Search STAC API — Complete Reference

Audited live on 2026-08-23 against `https://earth-search.aws.element84.com/v1`
(STAC 1.0.0, OGC API - Features). Everything below was verified with real
requests. This is the reference for strengthening `geocr_mcp`.

## 1. API surface

| Endpoint | Notes |
|---|---|
| `GET /` | Landing page; links to everything below |
...
| `GET /collections` | All 9 collections |
| `GET /collections/{id}` | Full collection: extent, license, providers, `item_assets`, `summaries` |
...
| `POST /search` | Everything above + `query` (property filters) |
| `GET /aggregations` | Lists `total_count`, `datetime_min/max`, `datetime_frequency` |
| `GET /aggregate` | **Broken** — returns empty `aggregations` for every query. Do not rely on it |

Key behaviors:

- **`query` silently drops items lacking the property.** e.g.
...
- `/search` accepts date-only ranges (`2023-07-01/2023-09-30`);
  `/aggregate` demands full RFC3339 — moot since aggregate is broken.
- Pagination: cursor `next` links; page size capped (100 max per request);
  ...
  free scene-count probe).

## 2. The 9 collections

| Collection | ~Items | Temporal | GSD (m) | Native EPSG | Storage | Requester pays |
|---|---|---|---|---|---|---|
| `sentinel-2-c1-l2a` | 29.9M | 2015-06→now | 10/20/60 | 326xx (UTM) | `e84-earth-search-sentinel-data`, us-west-2, **https COGs** | No |
| `sentinel-2-l2a` | 50.9M | 2015-06→now (legacy) | 10/20/60 | UTM | COG **and** `-jp2` assets | — |
| `sentinel-2-pre-c1-l2a` | 35k | frozen archive | 10/20/60 | UTM | same as c1 | — |
| `sentinel-2-l1c` | 44.3M | 2015-06→now (TOA) | 10/20/60 | UTM | `s3://sentinel-s2-l1c` JP2 | — |
| `sentinel-1-grd` | 3.7M | 2014-10→now | ~20x22 res / 10m px | 4326 | `s3://sentinel-s1-l1c` | **Yes** |
| `landsat-c2-l2` | 10.3M | 1982-08→now | 30/60/100/120 | UTM | `s3://usgs-landsat` | **Yes** |
...
| `naip` | 1.4M | 2010→2022 (US only) | 0.6/1 | 269xx (NAD83) | `s3://naip-analytic` | **Yes** |
...
- **Scene classification percentages** (flood-relevant!):
...
- c1 assets: https COG hrefs, `gsd`, `proj:shape`, `proj:transform`,
  ...
  SHA-256/multihash — don't emit it as `sha256`).
...
- **No `eo:cloud_cover`** (cloud-independent — that's its flood niche).
