...
// Displays a warning banner on older spec and documentation versions.

const LATEST_VERSION = "2026-08-31";
const DRAFT_VERSION = "draft";
const SECTIONS = [
  {
    pathRegex: /\/specification\/(\d{4}-\d{2}-\d{2}|draft|latest)(\/.*)?$/,
    latestAliasPath: "/specification/2026-08-31",
    storageKey: "geocr-latest-spec-version",
    noun: "specification",
    draftMessage: "You are viewing a draft of a not-yet-finalized specification.",
  },
  {
    pathRegex: /\/docs\/(\d{4}-\d{2}-\d{2}|draft|latest)(\/.*)?$/,
    latestAliasPath: "/docs/2026-08-31",
    storageKey: "geocr-latest-docs-version",
    noun: "documentation",
    draftMessage:
...
function insertWarningBanner() {
  ...
  if (current.version === LATEST_VERSION || current.version === "latest") {
    return;
  }
  ...
  const linkText = `View the latest version (${LATEST_VERSION})`;
  const linkHref = `${current.section.latestAliasPath}${current.subPath}`;
  ...
  content.innerHTML = `<span>${message} </span><a href="${linkHref}">${linkText}</a>`;
...
window.addEventListener("popstate", insertWarningBanner);
